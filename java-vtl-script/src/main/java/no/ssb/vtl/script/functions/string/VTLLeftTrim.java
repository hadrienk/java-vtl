...
public final class VTLLeftTrim extends AbstractVTLFunction<VTLString> {

    private static final Argument<VTLString> VALUE = new Argument<>("value", VTLString.class);
    private static VTLLeftTrim instance;

    public static VTLLeftTrim getInstance() {
        if (instance == null) {
            instance = new VTLLeftTrim();
        }
        return instance;
    }

    private VTLLeftTrim() {
        super("ltrim", VTLString.class, VALUE);
    }
